import '../ongoing_appointments_page/widgets/userprofile3_item_widget.dart';
import 'package:flutter/material.dart';
import 'package:medicon/core/app_export.dart';
import 'package:medicon/widgets/custom_text_form_field.dart';

class OngoingAppointmentsPage extends StatefulWidget {const OngoingAppointmentsPage({Key? key}) : super(key: key);

@override OngoingAppointmentsPageState createState() =>  OngoingAppointmentsPageState();

 }

// ignore_for_file: must_be_immutable
class OngoingAppointmentsPageState extends State<OngoingAppointmentsPage> with  AutomaticKeepAliveClientMixin<OngoingAppointmentsPage> {TextEditingController monthEditTextController = TextEditingController();
...
/// Section Widget
Widget _buildScrollView(BuildContext context) { return SingleChildScrollView(child: Column(children: [SizedBox(height: 35.v), Padding(padding: EdgeInsets.symmetric(horizontal: 16.h), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildMonthEditText(context), SizedBox(height: 39.v), Text("Ongoing Appointments", style: CustomTextStyles.titleMediumGray90002), SizedBox(height: 18.v), ListView.separated(physics: NeverScrollableScrollPhysics(), shrinkWrap: true, separatorBuilder: (context, index) {return SizedBox(height: 17.v);}, itemCount: 3, itemBuilder: (context, index) {return Userprofile3ItemWidget(onTapUserProfile: () {onTapUserProfile(context);});})]))])); } 
/// Navigates to the upcomingAppointmentOneScreen when the action is triggered.
onTapUserProfile(BuildContext context) { Navigator.pushNamed(context, AppRoutes.upcomingAppointmentOneScreen); } 
 }
